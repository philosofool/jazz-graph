/*
	Update Performance to albums.
	This collects distinct recordings from MB data
	grouped by album: one row per recording per album
	on which it has appeared.

	IMPORTANT CHANGE: Albums (which may duplicate
...
DROP VIEW IF EXISTS recording_to_album;
CREATE MATERIALIZED VIEW recording_to_album AS

...
I'm separating the requirements from this proposal so that the evaluation criteria are clear even if the end results are different from this proposal; a proposal is a good way to show that something is feasible. The following would, I think, satisfy the requirements very well and I believe that this can be completed.

1. (Data Model) There are entities: artists, songs and performances. There are relationships: artists _compose_ songs; artists _perform_ performances; a performance _plays_ a song. This creates a heterogeneous graph of interconnected entities representing a significant component of jazz collaboration.
...
For example, it won't include that there are numerous different performers with Thelonious Monk on "Genius of Modern Music," which was a compilation of five different recording sets, no two of which included the exact same performers.
